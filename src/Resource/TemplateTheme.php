 ...
 * @internal
 */
class TemplateTheme extends EntityQueryResourceBase {
  ...
  public function process(Request $request): ResourceResponse {
    ...
    $request->query->set('include', $include . (empty($include) ? '' : ',') . 'logo');
    $cacheability = new CacheableMetadata();

    // try to load theme;
    $confTheme = ConfigDrupal::config('system.theme');
    $entity_query = $this->getEntityQuery('config_theme_entity')->condition('hostname', $confTheme['default']);
