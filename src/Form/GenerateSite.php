...
class GenerateSite extends ConfigFormBase {
   ...
   */
  public function __construct(ConfigFactoryInterface $config_factory, DomainNegotiator $DomainNegotiator, ExportEntities $ExportEntities, FileSystem $FileSystem, ArchiverManager $ArchiverManager, ExtensionPathResolver $path_resolver) {
  ...
  public static function create(ContainerInterface $container) {
    return new static($container->get('config.factory'), $container->get('domain.negotiator'), $container->get('export_import_entities.export.entites'), $container->get('file_system'), $container->get(
      'plugin.manager.archiver'), $container->get("extension.path.resolver"));
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $domaineId = null) {
    ...
    if (!$form_state->has('step')) {
      ...
      // On le supprime afin d'avoir toujours un env OK.
      if (empty($form_state->getValues()) && file_exists($path)) {
        // $Filesystem = new FilesystemSymphony();
        // $Filesystem->remove($path);
        // $this->messenger()->addStatus(":: " . $form_state->get("create_dir"),
        // true);
      }
    }
    if (!file_exists($path)) {
      ...
      if (file_exists($pathTheme)) {
        ...
        $wbuAtomiquePath = $path . '/web/' . $subPathTheme . '/wbu-atomique-theme';
        $this->messenger()->addMessage("wbu path: " . $wbuAtomiquePath);
        if (file_exists($wbuAtomiquePath)) {
          $Filesystem->remove($wbuAtomiquePath);
        }
  ...
  public function nextSubmit(array &$form, FormStateInterface $form_state) {
    ...
    if ($form_state->getValue('generate_files')) {
      ...
      $this->ExportEntities->deleteConfigs();
